...
function icon(passed: boolean, severity: string): string {
...
function renderSuite(suite: SuiteResult, index: number): string {
  const lines: string[] = [];
  ...
  const parts: string[] = [];
  if (suite.passed > 0) parts.push(`${GREEN}${suite.passed} passed${RESET}`);
  if (suite.failed > 0) parts.push(`${RED}${suite.failed} failed${RESET}`);
  if (suite.warnings > 0)
 ...
 */
export function renderText(report: ContractReport): string {
  const lines: string[] = [];
  ...
  if (report.exitCode === 0) {
    ...
    lines.push(
      `${RED}${BOLD}❌ Contract violations found — fix before shipping${RESET}`,
    );
  }
 ...
 */
export function renderJSON(report: ContractReport): string {
 ...
 */
export function buildReport(suites: SuiteResult[]): ContractReport {
  let totalPassed = 0;
  let totalFailed = 0;
  let totalWarnings = 0;
