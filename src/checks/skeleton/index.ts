...
export function renderSkeletons(
  ...
  for (const sec of sections) {
    ...
    if (sec.found) {
      ...
      lines.push(
        `⚠️ Section not found in prototype — extract manually from \`${protoPath}\``,
      );
    }
    lines.push("");
    lines.push("---");
...
export function runSkeleton(
    ...
    checks.push(
      check(
        ...
        sec.found
          ...
          : `${sec.name} not found in prototype`,
        sec.found ? "info" : "warning",
      ),
    );
  }
  ...
  const outputPath = resolve(
    projectRoot,
    config.skeleton?.output ?? "component-skeletons.md",
  );
