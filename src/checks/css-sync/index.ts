...
export function extractPrototypeClassNames(protoPath: string): string[] {
  if (!existsSync(protoPath)) {
    throw new Error(`Prototype not found: ${protoPath}`);
  }
...
// ─── Extraction: inline style="..." CSS custom properties ───
...
export function extractInlineTokens(protoPath: string): InlineToken[] {
  if (!existsSync(protoPath)) {
    throw new Error(`Prototype not found: ${protoPath}`);
  }
...
export function extractJsThemeTokens(
  protoPath: string,
): Record<string, InlineToken[]> {
  if (!existsSync(protoPath)) {
    throw new Error(`Prototype not found: ${protoPath}`);
  }

  const html = readFileSync(protoPath, "utf-8");
  const result: Record<string, InlineToken[]> = {};

  // Find THEME = { ... } using brace counting
  const themeStart = html.indexOf("THEME");
  if (themeStart === -1) return result;

  const eqIndex = html.indexOf("{", themeStart);
  if (eqIndex === -1) return result;
...
export function tokenExistsInCss(
  tokenName: string,
  expectedValue: string,
  cssPath: string,
  selectorBlock?: string,
): { exists: boolean; actualValue: string | null } {
  if (!existsSync(cssPath)) return { exists: false, actualValue: null };

  const css = readFileSync(cssPath, "utf-8");
  let searchRegion = css;

  if (selectorBlock) {
    const escapedSelector = selectorBlock.replace(
      /[.*+?^${}()|[\]\\]/g,
      "\\$&",
    );
  ...
  const escapedName = tokenName.replace(
    /[.*+?^${}()|[\]\\]/g,
    "\\$&",
  );
  ...
  if (!tokenMatch) return { exists: false, actualValue: null };
...
export function runCssSync(
  ...
  for (const [themeName, tokens] of Object.entries(jsThemeTokens)) {
    let selectorBlock: string | undefined;
    if (themeName === "day") {
      selectorBlock = ":root";
    ...
    for (const token of tokens) {
      ...
      checks.push(
        check(
          `css-sync:theme:${themeName}:${token.name}`,
          checkName,
          result.exists,
          result.exists
            ? `${token.name}: ${token.value} — matches ${themeName} block`
            : `${token.name}: expected "${token.value}" in ${themeName} but ${
                result.actualValue
