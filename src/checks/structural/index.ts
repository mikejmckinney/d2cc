...
function fileMatchesRegex(filePath: string, pattern: RegExp): boolean {
  if (!existsSync(filePath)) return false;
  const content = readFileSync(filePath, "utf-8");
...
export function runStructural(
  ...
  if (config.structural?.patterns) {
    for (const [id, { file, pattern }] of Object.entries(
      config.structural.patterns,
    )) {
    ...
    for (const [name, path] of Object.entries(config.structural.svgPaths)) {
      ...
      try {
        ...
        if (existsSync(compDir)) {
          const files = readdirSync(compDir).filter((f: string) =>
            f.endsWith(".tsx"),
          );
